...
/** Mini-transcribe invents Tom/Visitor exhibit scripts from the session prompt on room tone. */
function isSttRoleplayHallucination(transcript) {
  ...
  return lower.includes("tom:") && (lower.includes("visitor:") || lower.includes("godfrey:"));
}
...
function attachUnrealSttWebSocket(server, { openaiApiKey } = {}) {
  ...
  wss.on("connection", (clientWs) => {
    ...
    let acceptTurnsSinceResume = false;
    ...
    openaiWs.on("message", (data) => {
      ...
      if (type === "input_audio_buffer.speech_started") {
        acceptTurnsSinceResume = true;
        sendJson(clientWs, { type: "speech_started", itemId: event.item_id || null });
        return;
      }
      if (type === "input_audio_buffer.speech_stopped") {
        if (!acceptTurnsSinceResume) {
          console.log("unreal-stt: ignored orphan speech_stopped (no speech_started since resume)");
          return;
        }
      ...
      if (type === "conversation.item.input_audio_transcription.completed") {
        ...
        if (isSttRoleplayHallucination(transcript)) {
          console.warn("unreal-stt: dropping roleplay-hallucination transcript", { transcript });
          sendJson(clientWs, {
            type: "transcript_missed",
            itemId: event.item_id || null,
            reason: "hallucination",
        ...
        if (!acceptTurnsSinceResume) {
          console.warn("unreal-stt: dropping stale transcript (no speech_started since resume)", {
            transcript,
          });
          return;
        }
