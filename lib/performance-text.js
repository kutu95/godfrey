...
/**
 * Index of a trailing partial word that must not be sent to TTS yet.
 * Whole-word pronunciation rewrites (`Vasse` → `Vas`) fail if a name is split
 * across LLM deltas. Returns -1 when the buffer already ends on a word boundary.
 */
function trailingPartialWordIndex(text) {
  if (!text) {
    return -1;
  }
  // * and ] close performance cues; do not treat a finished *cue* as a partial word.
  if (/[\s.,;:!?…"'”’)*\]—–-]$/.test(text)) {
    return -1;
  }
  const boundary = text.match(/[\s—–.,;:!?…"'”’)*\]-]+(?=[^\s—–.,;:!?…"'”’)*\]-]*$)/);
  const holdAt = boundary ? boundary.index + boundary[0].length : 0;
  const trailing = text.slice(holdAt);
 ...
 * sends and half a `[gesture:...]` can never be spoken aloud. A trailing partial
 * word is held the same way so TTS respellings see complete names.
...
function createStreamingCueStripper() {
  ...
  return {
    push(chunk) {
      ...
      let safe = cut === -1 ? pending : pending.slice(0, cut);
      pending = cut === -1 ? "" : pending.slice(cut);
      if (!pending) {
        const holdAt = trailingPartialWordIndex(safe);
        if (holdAt >= 0) {
          pending = safe.slice(holdAt);
          safe = safe.slice(0, holdAt);
        }
      }
...
Aye.`;
  ...
  const placeSource = "Loaded jarrah at Bunbury, then left the Vasse at nine.";
  for (const size of [1, 2, 3, 4]) {
    const stripper = createStreamingCueStripper();
    let out = "";
    for (let i = 0; i < placeSource.length; i += size) {
      out += stripper.push(placeSource.slice(i, i + size));
    }
    out += stripper.flush();
    const normalise = (s) => s.replace(/\s+/g, " ").trim();
    const batch = normalise(stripPerformanceCues(placeSource));
    if (normalise(out) !== batch || /Vasse|\bjarrah\b/i.test(out) || !/Jar-uh/.test(out) || !/\bVas\b/.test(out)) {
      console.error("streaming stripper missed a TTS respelling", {
        size,
        streamed: normalise(out),
        batch,
      });
