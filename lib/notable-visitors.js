...
function matchGivenName(text, entry) {
  return Boolean(findMatchingGivenName(text, entry));
}

/** The watchlist given-name phrase that matched, or null. */
function findMatchingGivenName(text, entry) {
  const haystack = normalizePersonName(text);
  if (!haystack || !entry) {
    return null;
  }
  return entry.givenNames.find((name) => containsPhrase(haystack, name)) || null;
}
