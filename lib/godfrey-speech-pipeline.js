...
// will emit any audio. First bucket 80 (was 50) so the opening clause of Welcome
// is one TTS flush instead of a 50-char cut mid-"Fremantle".
const CHUNK_LENGTH_SCHEDULE = [80, 120, 160, 290];
const WS_OPEN_TIMEOUT_MS = 5000;
const WS_FINISH_TIMEOUT_MS = 45000;
/** Abort a hung Responses stream so we can flush TTS instead of leaving Unreal with a half-spoken reply. */
const LLM_IDLE_TIMEOUT_MS = 8000;
...
async function streamGodfreyReplyToPcm({
  ...
  let llmStream = null;
  let llmIdleTimedOut = false;
  let lastLlmEventAt = Date.now();
  let llmIdleWatch = null;

  try {
    llmStream = await openai.responses.create({ ...requestParams, stream: true });
    lastLlmEventAt = Date.now();
    llmIdleWatch = setInterval(() => {
      if (!llmIdleTimedOut && Date.now() - lastLlmEventAt >= LLM_IDLE_TIMEOUT_MS) {
        llmIdleTimedOut = true;
        timing?.log?.("llm_idle_timeout", {
          idleMs: Date.now() - lastLlmEventAt,
          assistantChars: assistantText.length,
          totalPcmBytes,
        });
        console.warn("stream-pcm pipeline: LLM stream idle timeout — flushing TTS with text so far", {
          idleMs: Date.now() - lastLlmEventAt,
          assistantChars: assistantText.length,
          totalPcmBytes,
        });
    ...
    if (llmIdleTimedOut && (assistantText || totalPcmBytes > 0)) {
      // Keep the TTS socket open and flush what we have.
    } else {
      try {
        ws.close();
      } catch {
